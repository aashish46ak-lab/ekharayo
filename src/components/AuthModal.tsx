...
import { 
  Mail, ShieldCheck, Loader2, Eye, EyeOff, Lock, User as UserIcon, X,
  Leaf, Truck, BadgeCheck
} from "lucide-react";
...
const AuthModal = () => {
  ...
  const field = "w-full border border-border rounded-lg pl-10 pr-4 py-3 font-body text-sm bg-muted text-foreground focus:outline-none focus:ring-2 focus:ring-ring placeholder:text-muted-foreground";
  const btn = "w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground font-body font-semibold py-3.5 rounded-lg hover:bg-green-glow transition-colors disabled:opacity-60";
  ...
  return (
    <Dialog open={isAuthModalOpen} onOpenChange={(open) => !open && closeAuthModal()}>
      <DialogContent className="p-0 sm:max-w-[900px] max-w-[95vw] overflow-hidden border-none bg-background rounded-2xl shadow-2xl">
        <div className="flex h-[600px] max-h-[90vh]">
          {/* Brand side */}
          <div className="hidden md:flex md:w-[45%] relative overflow-hidden bg-gradient-to-br from-primary/20 via-background to-secondary items-center justify-center p-8 border-r border-border">
            <div className="absolute inset-0 opacity-20 bg-[linear-gradient(hsl(var(--primary)/0.3)_1px,transparent_1px),linear-gradient(90deg,hsl(var(--primary)/0.3)_1px,transparent_1px)] bg-[size:32px_32px]" />
            <div className="relative space-y-6">
              <img src={logo} alt="eKharayo" className="h-12 w-auto" />
              <div>
              ...
              <ul className="space-y-4">
                {[
                  { icon: Leaf, text: "Farm-fresh dairy & crops" },
                  { icon: Truck, text: "Reliable home delivery" },
                  { icon: BadgeCheck, text: "Secure verified accounts" },
                ].map((f) => (
                  <li key={f.text} className="flex items-center gap-3 font-body text-xs text-foreground">
          ...
          <div className="flex-1 flex flex-col p-8 overflow-y-auto">
            ...
            <div className="max-w-sm mx-auto w-full my-auto py-4">
              <img src={logo} alt="eKharayo" className="h-10 w-auto mx-auto mb-6 md:hidden" />
                ...
                <form onSubmit={login} className="space-y-4">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <button 
                      type="button"
                      onClick={handleGuest}
                      className="w-full font-body text-sm text-muted-foreground hover:text-primary transition-colors py-2 border border-dashed border-border rounded-lg"
                    >
                      Continue as Guest
                    </button>
                  </div>
              ...
              {mode === "otp" && (
                <form onSubmit={verify} className="space-y-4">
                  <input
                    ...
                    placeholder="123456"
                    className="w-full border border-border rounded-lg px-4 py-3 font-body text-center text-2xl tracking-[0.5em] bg-muted text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                  />
