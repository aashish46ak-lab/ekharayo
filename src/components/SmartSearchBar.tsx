...
const SmartSearchBar = ({ variant = "hero" }: SmartSearchBarProps) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown") {
      ...
      else if (results.length > 0) goTo(results[0].href);
      else if (query.trim()) goTo("/products");
  ...
  return (
    <div ref={containerRef} className={`relative w-full ${isHero ? "max-w-xl mx-auto" : "w-full max-w-[220px] lg:max-w-xs"}`}>
      <div
    ...
    for (const item of items) {
      for (const kw of item.keywords) {
        ...
        if (Math.abs(kwLower.length - q.length) <= 2) {
          ...
          for (let i = 0; i < len; i++) if (kwLower[i] !== q[i]) diff++;
