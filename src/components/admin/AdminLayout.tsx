    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      <header className="sticky top-0 z-40 bg-card border-b border-border shadow-sm">
        <div className="flex items-center gap-3 px-3 sm:px-4 h-12">
          <button type="button" className="lg:hidden p-1.5 rounded-md hover:bg-secondary" onClick={() => setOpen(true)} aria-label="Open menu">
            <Menu size={20} />
          </button>
          ...
          <Link to="/admin/orders" className="relative p-2 rounded-md hover:bg-secondary text-foreground" title="Orders">
            <ShoppingBag size={18} />
            {ordersBadge > 0 && (
          ...
          <Link to="/admin/messages" className="relative p-2 rounded-md hover:bg-secondary text-foreground" title="Messages">
          ...
          <Link to="/admin/notifications" className="relative p-2 rounded-md hover:bg-secondary text-foreground" title="Notifications">
            <Bell size={18} />
            {notifsBadge > 0 && (
          ...
          <span className="hidden md:inline font-body text-xs text-muted-foreground truncate max-w-[160px]">{user?.email}</span>
        </div>
      </header>

      <div className="flex flex-1 min-h-0">
        <aside
          className={`fixed lg:static inset-y-0 left-0 z-50 w-60 bg-card border-r border-border flex-col pt-12 lg:pt-0 ${
        ...
        >
          <div className="p-3 border-b border-border flex items-center justify-between lg:hidden">
            ...
            <button type="button" className="p-1 rounded-md hover:bg-secondary" onClick={() => setOpen(false)} aria-label="Close">
              <X size={18} />
            </button>
          </div>
            ...
            {links.map((l) => {
              ...
              return (
                <NavLink key={l.to} to={l.to} end={l.end} onClick={() => setOpen(false)} className={({ isActive }) => item(isActive)}>
                  ...
                  {count > 0 && (
                    <span className="min-w-[20px] h-5 px-1.5 rounded-full bg-primary/15 text-primary text-[10px] font-bold flex items-center justify-center">
                      {count > 99 ? "99+" : count}
                    </span>
                  )}
                </NavLink>
        ...
        <main className="flex-1 min-w-0 p-3 sm:p-5 lg:p-6 overflow-x-hidden bg-background">
