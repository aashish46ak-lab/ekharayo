...
const ProductDetail = () => {
  ...
  const addToCart = () => {
    ...
    add(
      { id: product.id, name: product.name, price, image: product.images?.[0] ?? null, unit: product.unit },
      qty,
    );
  };
    ...
    <div className="min-h-screen pt-14">
      ...
      <div className="container mx-auto px-4 py-10 max-w-5xl">
        <button
          type="button"
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-2 text-muted-foreground hover:text-primary font-body text-sm mb-6"
        >
          ...
          <>
            <div className="grid md:grid-cols-2 gap-8 mb-16">
              ...
              <div className="flex flex-col">
                <div className="flex items-start justify-between gap-3 mb-2">
                  ...
                  <button
                    ...
                    className={`p-2 rounded-full border transition-colors ${
                      has(product.id) ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:text-primary"
                    }`}
                    aria-label="Wishlist"
                  >
                ...
                {product.featured && (
                  <span className="w-fit font-body text-[10px] uppercase tracking-wide bg-accent/15 text-accent px-2 py-1 rounded-full mb-3">
                    Featured
                  </span>
                )}
                <div className="flex items-baseline gap-2 mb-4">
                  ...
                  {product.sale_price != null && (
                    <span className="font-body text-sm text-muted-foreground line-through">{rs(Number(product.price))}</span>
                  )}
                ...
                <p className="font-body text-sm text-muted-foreground mb-2">
                  {product.stock > 0 ? (
                    product.stock <= 5 ? (
                      <span className="text-accent font-medium">Only {product.stock} left in stock</span>
                    ) : (
                ...
                {product.description && (
                  <p className="font-body text-sm text-foreground/80 leading-relaxed mb-6 whitespace-pre-line">
                    {product.description}
                  </p>
                )}
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {related.map((r) => (
                    <Link
                      key={r.id}
                      to={`/products/${r.id}`}
                      className="bg-card border border-border rounded-xl overflow-hidden hover:border-primary/40 hover:-translate-y-0.5 transition-all"
                    >
                      {r.images?.[0] ? (
                        <img src={r.images[0]} alt={r.name} className="w-full h-32 object-cover" />
                      ) : (
